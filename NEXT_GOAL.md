...
- Generated: 2026-06-21T20:14:29Z
- Current toolkit version: 3.131.0
- Target release: v3.169.0
- Title: Stable V4 GitHub Release Metadata Closure Kit QA
...
- Completed scope: v3.168 added downloaded release-asset closure-kit details to stable-publication reports: downloaded-release-assets blockers now expose required release assets, GitHub metadata asset names and missing assets, downloaded or supplied local asset names and missing assets, download source/status, asset directory, repair/pass/fail criteria, download/supplied-assets rerun command, full stable-publication rerun command, and metadata-only/source-only/fixture-proof boundaries; report-quality now enforces those fields and a public fixture proves the release-asset closure row without treating source, metadata-only, or fixture assets as stable-v4 proof.
- Evidence: python3 -m py_compile scripts/v4_stable_publication.py scripts/ios_report_quality.py; git diff --check; ./tests/v4_stable_publication_test.sh; ./tests/ios_report_quality_test.sh; ./bin/shipguard validate; ./bin/shipguard docs-check . --out /tmp/shipguard-v3168-docs-check; ./tests/self_audit_test.sh; ./tests/cli_smoke_test.sh; ./tests/package_release_test.sh; ./bin/shipguard value-gauntlet --path . --out /tmp/shipguard-v3168-value-gauntlet; codex plugin marketplace add .; codex plugin add ios-shipguard@shipguard; ./bin/shipguard codex status --strict
...
```bash
./bin/shipguard next-goal --release 3.170.0 --title "Stable V4 Published Release Metadata Closure Kit QA" --out NEXT_GOAL.md
```
