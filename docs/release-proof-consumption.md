...
For CI, use `jlekerli-source/ShipGuard/actions/release-consume@v3.163.0` to download the assets, run this verification, upload the consumer proof bundle, and fail the job when proof is invalid. Use `jlekerli-source/ShipGuard/actions/release-evidence@v3.163.0` afterward when you want a static HTML evidence artifact.

Download the release assets from GitHub:

```bash
mkdir -p /tmp/shipguard-v3.163.0
gh release download v3.163.0 \
  --repo jlekerli-source/ShipGuard \
  --pattern 'shipguard-v3.163.0.tar.gz' \
...
./bin/shipguard release-consume verify \
  --dir /tmp/shipguard-v3.163.0 \
  --out /tmp/shipguard-v3.163.0/consumer-proof \
  --version 3.163.0
```
...
```bash
shasum -a 256 /tmp/shipguard-v3.163.0/shipguard-v3.163.0.tar.gz
```
...
./bin/shipguard release-replay verify \
  --manifest /tmp/shipguard-v3.163.0/release-manifest.json \
  --tarball /tmp/shipguard-v3.163.0/shipguard-v3.163.0.tar.gz \
  --index /tmp/shipguard-v3.163.0/release-index.json \
  --ledger /tmp/shipguard-v3.163.0/proof-ledger.md \
  --out /tmp/shipguard-v3.163.0/consumer-replay
```
...
./bin/shipguard release-attest build \
  --manifest /tmp/shipguard-v3.163.0/release-manifest.json \
  --replay /tmp/shipguard-v3.163.0/consumer-replay/replay-report.json \
  --out /tmp/shipguard-v3.163.0/consumer-attestation
```

Review these files:

- `/tmp/shipguard-v3.163.0/consumer-proof/consumer-report.json`
- `/tmp/shipguard-v3.163.0/consumer-proof/consumer-report.md`
- `/tmp/shipguard-v3.163.0/consumer-proof/asset-digests.json`
- `/tmp/shipguard-v3.163.0/consumer-proof/asset-digests.md`
- `/tmp/shipguard-v3.163.0/consumer-proof/replay/replay-report.json`
- `/tmp/shipguard-v3.163.0/consumer-proof/replay/replay-report.md`
- `/tmp/shipguard-v3.163.0/consumer-proof/attestation/attestation.json`
- `/tmp/shipguard-v3.163.0/consumer-proof/attestation/attestation.md`
- `/tmp/shipguard-v3.163.0/consumer-proof/attestation/attestation-badge.json`

Optionally export a static evidence page:

```bash
./bin/shipguard release-evidence site \
  --consume /tmp/shipguard-v3.163.0/consumer-proof \
  --out /tmp/shipguard-v3.163.0/evidence-site
```
...
./bin/shipguard release-evidence bundle \
  --assets /tmp/shipguard-v3.163.0 \
  --left /tmp/shipguard-v3.119.0 \
  --out /tmp/shipguard-v3.163.0-evidence-bundle \
  --version 3.163.0
```
...
- `attestation-badge.json` says `pass v3.163.0`.
