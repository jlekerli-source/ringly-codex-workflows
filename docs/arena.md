...
The public fixture pack includes clean, weak, dangerous, failing-validation, missing-diff, review-only, backend webhook, frontend async-state, docs release-proof drift, CLI cleanup, security token-leakage, GitHub posting without dry-run, release asset trust-bypass, and StoreKit entitlement regression maintainer runs so the benchmark shows multiple evidence and failure modes.
