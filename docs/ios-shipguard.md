...
The `ios performance` report is read-only. It scans Swift source for ranked hotspots such as high-frequency `TimelineView`, continuous `repeatForever` animation, large blur/shadow composition, formatter allocation in SwiftUI paths, image decoding in UI paths, and notification cleanup that may block launch or interaction. Each finding includes an impact explanation that is shown in Markdown as "Why it matters", so private-app product-QA runs can prioritize ShipGuard improvements without relying on hidden app context. iOS source scanners skip generated/proof/cache directories and list those scan-scope exclusions in JSON and Markdown so large app workspaces do not turn release artifacts or scratch files into product findings.
