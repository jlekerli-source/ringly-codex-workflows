...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ShipGuard/actions/validate@v3.113.0
```
...
- name: Compare Arena results
  uses: jlekerli-source/ShipGuard/actions/arena-compare@v3.113.0
  with:
...
- name: Verify redacted transcript
  uses: jlekerli-source/ShipGuard/actions/transcript-verify@v3.113.0
  with:
...
- name: Verify transcript corpus
  uses: jlekerli-source/ShipGuard/actions/transcript-corpus@v3.113.0
  with:
...
- name: Build release proof
  uses: jlekerli-source/ShipGuard/actions/release-proof@v3.113.0
  with:
...
- name: Verify published proof assets
  uses: jlekerli-source/ShipGuard/actions/release-consume@v3.113.0
  with:
    repo: jlekerli-source/ShipGuard
    release-tag: v3.113.0
    mode: fail
```
...
- name: Compare published proof assets
  uses: jlekerli-source/ShipGuard/actions/release-diff@v3.113.0
  with:
    repo: jlekerli-source/ShipGuard
    left-tag: v0.0.0
    right-tag: v3.113.0
...
- name: Export release evidence
  uses: jlekerli-source/ShipGuard/actions/release-evidence@v3.113.0
  with:
...
- name: Verify release evidence artifact
  uses: jlekerli-source/ShipGuard/actions/release-evidence-verify@v3.113.0
  with:
...
- name: Audit release evidence negative fixtures
  uses: jlekerli-source/ShipGuard/actions/release-evidence-negative-index@v3.113.0
  with:
    mode: fail
```
