...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ShipGuard/actions/validate@v3.94.0
```
...
- name: Build release proof
  uses: jlekerli-source/ShipGuard/actions/release-proof@v3.94.0
  with:
...
- name: Verify published proof assets
  uses: jlekerli-source/ShipGuard/actions/release-consume@v3.94.0
  with:
...
- name: Compare published proof assets
  uses: jlekerli-source/ShipGuard/actions/release-diff@v3.94.0
  with:
    repo: jlekerli-source/ShipGuard
    left-tag: v0.0.0
    right-tag: v3.94.0
...
- name: Export release evidence
  uses: jlekerli-source/ShipGuard/actions/release-evidence@v3.94.0
  with:
...
- name: Audit release evidence negative fixtures
  uses: jlekerli-source/ShipGuard/actions/release-evidence-negative-index@v3.94.0
  with:
    mode: fail
```
