...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ShipGuard/actions/validate@v3.59.0
```
...
- name: Compare Arena results
  uses: jlekerli-source/ShipGuard/actions/arena-compare@v3.59.0
  with:
...
- name: Verify redacted transcript
  uses: jlekerli-source/ShipGuard/actions/transcript-verify@v3.59.0
  with:
...
- name: Verify transcript corpus
  uses: jlekerli-source/ShipGuard/actions/transcript-corpus@v3.59.0
  with:
...
- name: Build release proof
  uses: jlekerli-source/ShipGuard/actions/release-proof@v3.59.0
  with:
...
- name: Compare published proof assets
  uses: jlekerli-source/ShipGuard/actions/release-diff@v3.59.0
  with:
    repo: jlekerli-source/ShipGuard
    left-tag: v0.0.0
    right-tag: v3.59.0
...
- name: Audit release evidence negative fixtures
  uses: jlekerli-source/ShipGuard/actions/release-evidence-negative-index@v3.59.0
  with:
    mode: fail
```
